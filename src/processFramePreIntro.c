...
typedef struct {
	int Line, Column, LetterFrame, LetterFrames;

} PreInroStruct;

PreInroStruct gPreIntro = { .Line = 1, .Column = 1, .LetterFrames = 2 };



int GamePreIntroStartTextGetTile(int tileIndex) {
	return tileIndex == 32 ? 0 : tileIndex; // +(12 * 16);
}


void GamePreIntroFrame() {
	...
	int tt_x = 0, tt_y = 2, currentChar = 0;
	for (int yy = 0; yy < gPreIntro.Line; yy++) {
		tt_x = 0;
		int columnToFill = (yy == gPreIntro.Line - 1) ? gPreIntro.Column : PREINTRO_COLUMNS;
		for (int tt = 0; tt < columnToFill; tt++) {
			currentChar = gPreIntroMessage[yy][tt];
			TileDrawCharPos(GamePreIntroStartTextGetTile(currentChar /*+ (12 * 16)*/), tt_x, tt_y + yy);
			tt_x++;
		}
	}

	// Fill out text gradually
	if (gPreIntro.Line < PREINTRO_LINES) {
		gPreIntro.LetterFrame++;
		if (gPreIntro.LetterFrame == gPreIntro.LetterFrames) {
			gPreIntro.LetterFrame = 0;
			gPreIntro.Column++;		

			// Wrap to next line
			if (gPreIntro.Column == PREINTRO_COLUMNS) {
				gPreIntro.Column = 0;
				gPreIntro.Line++;
			}

			if (currentChar != 32 && gPreIntro.Column != 1) {
				SoundPlayEmpty();
				//SoundPlayReload();
				//SoundPlayKill();
			}
	...
	if (gPreIntro.Line == PREINTRO_LINES && gInputFire) {
